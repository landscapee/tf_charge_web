...
export default{
	data(){
		...
		var validatePassword = (rule, value, callback) => {
			if (value === '' || value === null) {
				callback(new Error('该值不能为空'));
			} else if(!/^(?=.*[a-z].*)(?=.*[A-Z].*)(?=.*[0-9].*)[a-zA-Z0-9-*/+.~!@#$%^&*()]{8,16}$/.test(value)){
	...
	methods:{
		...
		changePassword(){
			...
			this.$refs['configPassword'].validate((valid) => {
				if (valid) {
					this.$axios.post("/sys/user/changeUserPwd",{
						id:this.userData.id,
						oldPwd:this.coder?encryptedData(this.passwordObj.oldPwd):this.passwordObj.oldPwd,
						newPwd:this.coder?encryptedData(this.passwordObj.newPwd):this.passwordObj.newPwd
					})
                    .then((res)=>{
		...
		passwordAlert(){
			...
			if(!/^(?=.*[a-z].*)(?=.*[A-Z].*)(?=.*[0-9].*)[a-zA-Z0-9-*/+.~!@#$%^&*()]{8,16}$/.test(this.passwordObj.newPwd)){
				this.$alert("请输入8-16位密码，至少包含大小写字母以及数字，可以包含特殊字符。例如：Aabc1234", '提示', {
					type: 'error',
					center: true
				})
				return false;
...
#com_glob_head{height: 48px;width: 100%;background:linear-gradient(90deg,#183D71 0%,#0172CF 100%);display: flex;align-items: center;justify-content: space-between;;
	.logo_left{height: 48px;;display: flex;justify-content:space-around;align-items: center;background:linear-gradient(90deg,#0F4291 0%,#428BFF 100%);box-shadow:0px 0px 6px 0px rgba(47,61,142,1);border-radius:0px 0 100px 0px;color: #fff;padding:0 20px;;
		
		div{height:100%;width:110px;;background-size:100% auto;background-repeat: no-repeat;background-position:center left;;}
		span{font-size:18px ;margin-left:15px;}
	}
	.nav_middle{display: flex;align-items: center;margin-left: 22px;flex: 1;;
		li{display: flex;justify-content: center;align-items: center;height: 48px;cursor: pointer;;
	...
	.user_right{display: flex;align-items: center;
		&>div{margin:0 12px ;height: 40px;display: flex;justify-content: flex-start;align-items: center;;

			span{color:#fff;font-size:16px;margin-left:8px;}
		}
		.timeBox{width:80px;
			span{
				font-size:18px;
		...
		.logoutBox,.setBox{width:48px;height:48px;background:rgba(14,52,113,1);opacity:0.6;cursor: pointer;
			
		}
		.logoutBox{margin:0;border-left: 1px solid rgba(255,255,255,.3);justify-content: center;}
	}
}
</style>
<style lang="scss">
.handover_content{
	h3{font-weight: 600;}
	.left_Box{width:260px;height: 400px;border: 1px solid #ddd;margin-right:15px ;padding: 10px;overflow: auto;;
		
		.treeBox{flex:1;padding: 10px 0;}
	}
	.right_Box{flex:1;border: 1px solid #ddd;padding: 10px;height: 400px;overflow: auto;;
		.userBox{flex:1;padding: 10px 0;
