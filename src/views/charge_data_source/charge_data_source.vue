<template>
    <div id="project" class="rightBoxContent">
        ...
        <div id="tableBox">
            <div class="tableBox" ref="tableBox">
                <el-table :data="lists" border stripe style="width: 100%" :max-height="maxHeight" :highlight-current-row="true">
                    ...
                    <el-table-column prop="code" label="数据源编码"></el-table-column>
                    ...
                    <el-table-column label="类型">
                        <template slot-scope="scope">
                            {{getName(scope.row.type,dataTypeLists,'name')}}
                        </template>
                    </el-table-column>
                    <el-table-column label="参数" width="120" align="center">
                        <template slot-scope="scope">
                            <el-popover placement="right" width="400" trigger="hover">
                                <pre>
                                    {{scope.row.parameter?JSON.parse(scope.row.parameter):''}}
                                </pre>
                                <el-button slot="reference" type="primary" size="mini" v-if="getShowParameter(scope.row.parameter)">查看</el-button>
                            </el-popover>
                        </template>
                    </el-table-column>
        ...
        <edit-list ref="ref_editList" @update="update" :dataTypeLists="dataTypeLists"></edit-list>
    </div>
...
export default {
    ...
    methods: {
        ...
        getLists(data) {
            ...
            this.$axios
                .get('/charge-data-source/findChargeDataSourceWhitPage', {
                    params: data,
                })
                .then((res) => {
                    console.log(res)
        ...
        del(data) {
            this.$confirm('确定删除该数据?', '提示', {
                ...
                this.$axios
                    .get('/charge-data-source/deleteById', {
                        params: {
                            id: data.id,
                        },
                    })
