        ...
        <el-form label-position="right" label-width="90px" ref="listData">
            ...
            <el-form-item label="计量单位" required>
                <el-select v-model="listData.unit" placeholder="请选择">
                    <el-option v-for="(item,idx) in unitLists" :key="idx" :label="item.name" :value="item.code">
                    </el-option>
                </el-select>
            </el-form-item>
...
export default {
    ...
    methods: {
        ...
        save() {
            ...
            this.$axios.post('/charge-config/save', this.listData).then((res) => {
                console.log(res)
                this.listShow = false
        ...
        dataVerify() {
            ...
            if (!this.listData.unit) {
                this.$alert('计量单位不能为空！', '提示', {
                    type: 'error',
                    center: true,
                })
                return false
            }
            // if (!this.listData.input) {
            //     this.$alert('采集方式不能为空！', '提示', {
            //         type: 'error',
            //         center: true,
            //     })
            //     return false
